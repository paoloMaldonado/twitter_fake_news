...
def export_to_excel(df, directory_to_export, path, column):
    # path without "/"
    splitted_path = path.split("/")[6:] # the 6 is an special case for this local pc
    root_path     = path.split("/")[:6]
    # path without "data/"
    creation_path = '/'.join(root_path) + '/'+ directory_to_export +'/' + '/'.join(splitted_path[1:])
    # path without the name of the file 
    new_path      = '/'.join(root_path) + '/'+ directory_to_export +'/' + '/'.join(splitted_path[1:-1])
