...
def preprocess_tweet(path, content_column, sheet, stopwords, nlp_model=None, opt_replies=False, opt_lemmatization=False):
    # read file
    df_original = pd.read_excel(path, index_col=0, sheet_name=sheet)
    ...
    # cast row to str 
    df[content_column]=df[content_column].apply(str)
    ...
    df = df.assign(hashtags = df[content_column].apply(lambda x : find_hashtags(x)))
    
    # remove urls, mentions and emojis
    if opt_replies == True:
        p.set_options(p.OPT.URL, p.OPT.MENTION, p.OPT.SMILEY, p.OPT.HASHTAG)
    else:
        p.set_options(p.OPT.URL, p.OPT.MENTION, p.OPT.SMILEY)
    tw_data = clean_tweets(df, "hashtags", opt_replies)
